...
module hybrid_address::package_manager {
    use std::signer;
    ...
    /// Upgrades the contract to the new code, at the moment, only lets the deployer upgrade
    entry fun upgrade(
        ...
        code::publish_package_txn(resource_account_signer, package_metadata, code)
    }

    /// Can be called by friended modules to obtain the resource account signer.
    public(friend) fun get_signer(): signer acquires PermissionConfig {
        let signer_cap = &borrow_global<PermissionConfig>(@hybrid_address).signer_cap;
    ...
    public(friend) fun add_address(name: String, object: address) acquires PermissionConfig {
        let addresses = &mut borrow_global_mut<PermissionConfig>(@hybrid_address).addresses;
    ...
    public(friend) fun get_address(name: String): address acquires PermissionConfig {
        let addresses = &borrow_global<PermissionConfig>(@hybrid_address).addresses;
        *smart_table::borrow(addresses, name)
    }

    inline fun safe_permission_config(): &PermissionConfig acquires PermissionConfig {
        borrow_global<PermissionConfig>(@hybrid_address)
    }
